...
Canvas {
    ...
    onPaint: {
        ...
        case "export":  // 导出：托盘上箭头
            ...
            break
        case "import":  // 导入：落进托盘的下箭头
            p(); mv(.5, .14); ln(.5, .56); mv(.34, .4); ln(.5, .56); ln(.66, .4); stroke2()
            p(); mv(.2, .66); ln(.2, .82); ln(.8, .82); ln(.8, .66); stroke2()
            break
        case "update":  // 更新：下箭头 + 一条落定的横线（import 是 U 形托盘，两者同在图标栏，画重了就分不出）
            p(); mv(.5, .14); ln(.5, .60); mv(.33, .43); ln(.5, .60); ln(.67, .43); stroke2()
            p(); mv(.22, .79); ln(.78, .79); stroke2()
            break
        case "scan":  // 扫描：放大镜+波
