...
- 落点 ~/.qianbi_novel/usage/usage.jsonl，每行一条调用记录（ts/ymd/slot/model/in/out/latency）
...
def _new_day() -> dict:
    return {"in": 0, "out": 0, "calls": 0, "by_model": {}, "by_slot": {}}
...
def _bump(day: dict, model: str, slot: str, tin: int, tout: int):
    ...
    for key, sub in (("by_model", model or "unknown"), ("by_slot", slot or "unknown")):
        m = day[key].setdefault(sub, {"in": 0, "out": 0, "calls": 0})
        m["in"] += tin
        m["out"] += tout
        m["calls"] += 1
...
def record(cfg: dict, model: str, slot: str, tin: int, tout: int, latency: float = 0.0):
    ...
    with _lock:
        ...
        _bump(day, model, slot, int(tin), int(tout))
        ...
        for k in day_keys:
            ...
            for bucket_key in ("by_model", "by_slot"):
                for model, m in (d.get(bucket_key) or {}).items():
                    t = total[bucket_key].setdefault(model, {"in": 0, "out": 0, "calls": 0})
                    t["in"] += m["in"]
                    t["out"] += m["out"]
                    t["calls"] += m["calls"]
        return total
