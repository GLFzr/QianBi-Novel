...
- 两条任何一条没过，面板上**根本不会出现「立即安装」**——那份清单只会被显示出来给你看，
  应用不会照着它下载或执行任何东西。地址也强制 https，`file://` 与网络路径一律拒绝。
...
24 小时内不重复问，**不上传任何内容**（不带书名、不带配置、不带 Key），在面板里可关，
...
.venv/Scripts/python -m pytest tests/unit -q        # 480 项
...
| `probe_update_ui.py` | 更新链路 40 项（零真网络）：多通道回退与逐条死因、未验签不给安装按钮、离线导入清单、本机包对哈希、24h 限流、设置白名单、面板溢出 |
