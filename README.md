...
- 🎨 **9 套 v2 题材预设**：修仙/都市/克苏鲁/规则怪谈/无限流/末日废土/历史权谋/电竞系统/童话寓言，含 6 阶段特化提示
...
## 🌟 v0.13.0-dev 重大升级（2026-08-27 · TUI 优势功能完整移植）

本次升级把同源 TUI 项目中**优于 GUI 的 10 项核心功能**完整移植到 GUI 端，并通过**真实 LLM 5 章共写 e2e 验证**：

| 维度 | v0.12.0 | v0.13.0-dev | 提升 |
|---|---|---|---|
| **题材预设** | 2 套 v1 | **9 套 v2**（含 stage_hints 6 阶段特化） | **+350%** |
...
| **独立预设库面板** | 嵌在 NotesPanel | 侧栏**第 7 项**独立面板 + 6 阶段 hint 预览 | **UI 解耦** |
...
- **写作偏好**（章节字数目标/审校开关/运行模式/逐步确认/决策门 G1-G9）
- **外观**（编辑器字号 4 档/限宽/流式速度/3 主题）
- **快捷键表**（Ctrl+S/F5/Esc/←→/Ctrl+B/Ctrl+E/**Ctrl+T 切主题**）
- **预设库**（侧栏第 7 项独立面板，含导入/导出/浏览 9 套 v2 预设）
...
app/
  ...
  ui/
    ...
    qml/              # 界面（★ v0.13: +PresetLibraryPanel + ReviewIssueDialog + 3 主题 Theme）
      Main.qml          # 主壳（侧栏 7 面板 + Ctrl+T 主题切换）
      ...
      components/       # ReaderView（阅读器）+ CwDialogueDock（共写对话区）
                       # + ReviewIssueDialog（6 维审校弹窗）+ 18 个设计系统组件
...
| 维度 | v0.12.0 | v0.13.0-dev | 评价 |
|---|---|---|---|
| **题材预设** | 2 套 v1 | **9 套 v2**（含 stage_hints 6 阶段特化） | **+350% 丰富度** |
