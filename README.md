...
- 💰 **成本架构（v0.19.5）**：卷级会话栈（20 章命中率 93.8%+）+ 静态指令冻结 + 台账
  增量更新（每章上下文增量 +40k → +2k 近常数）+ 清算级联 + L0 账本对照（零 API 确定性
  预检）+ 离峰挂机 —— 长篇场景单章实付成本较 0.19.0 再降 30-56%，上下文增量逼近常数
- 🧭 **Agent 操作层（v0.19）**：在共写对话里说「回退到去味之前」「重写第2章，铺垫足一点」，
  Agent 真的去操作应用（断点回退/细纲重生成/章节重写/设置开关），破坏性操作先归档可恢复
...
- 安全纪律：破坏性操作**先归档再删除**（`pipeline_debug/agent_tools/` 可恢复）；
