...
| 2026-08-28 | 真实窗口全量测试 | ZCode | GUI aae4b5c / TUI e785099 | **《时间当铺》10 章完稿（18,042 字）**，真实 API（百炼 deepseek-v4-flash）：第 1 章 step 模式**七门全链真机触发**（G2 带想法继续/G5L/G4/G6 带想法/G7/G8 人工裁决/G9），Console 对话区全程留痕（session-2026 0828/0829.jsonl：gate×7 + user×2 + agent×18）；第 2-10 章 auto 模式无人值守连跑。**真机暴露并修复 1 个高危缺陷**（aae4b5c/e785099）：审校修复轮模型返回 `===REVISIONS===` 修订计划被当正文采纳 → 整章被替换为指令清单且复检误判改善（第 5 章残章 162 字）——双端补健全性守卫（拒修订计划/长度骤减），修复后第 5 章重生成 1914 字合格正文、第 11 章同场景守卫正确拦截。T4.1 验收「四门真机各触发一次」达成；T3.3 停止打断门等待真机验证达成 |
| 2026-08-29 | 真机发现的后续项（记录，不在本轮修） | ZCode | — | ① `next_chapter_num`=max+1：中间章「重写本章→开始」实际从末章+1 续跑，与提示语「从该章续跑」不符（本轮以 total_chapters 边界法绕过）；② 停止后 G9 决策条未隐藏（陈旧展示，纯 UI）；③ 6 维审校 verdict=REJECT/REJECT-HARD 但阻塞解析为 0 的方差（评审输出格式漂移，L1 方法论已知风险，G8 人工裁决兜底有效）；④ 第 6-8 章字数 1018-1589 低于目标（字数闸门「标记不阻断」策略下的已知宽松） |
| 2026-08-29 | 收尾提交 | ZCode | 见 git log | 计划文档回填完成；工作树干净；dual_sync_check 终态 0 意外漂移；GUI 单测 51/51；TUI smoke 12 项全绿。产物：`C:\Users\zsfzr\Documents\千笔一文\时间当铺`（正文 10 章 + 追踪 6 文件 + 设定/大纲 + console 会话留痕）；裁剪的第 11 章与残章备份在 `pipeline_debug/rollback/` |
| 2026-08-29 | 真机缺陷修复（四项） | ZCode | GUI 845bbb5 / TUI df46893 | 上表「后续项」①-④ 全部修复：① first_missing_chapter 从缺口续跑 + 循环跳过已定稿章（真机复验：删第 6 章后流水线精确从 6 续跑，7-10 跳过，自然完本）；② gateClosed 信号清残留决策条；③ 审校解析三级兜底（括号式判级修复——prompt 规定 `[fail]` 而旧实现只认裸词" fail "全部漏判是根因之一；markdown 维度扫描；verdict 关键词提取 + 未结构化合成，双端含 TUI 两代 parse_review_findings）；④ word_enrich_rounds 多轮扩写（默认 2）+ 扩写稿守卫（拒空/拒变短）。单测 59→64 |
| 2026-08-29 | AI 味语料分析（4 子代理） | ZCode | 分析报告（本表下方摘要） | 语料：改命笔记 54 章 / 时间铺子 10 章 / 青冥问道 5 章 / 时间当铺 10 章，全部命令计数。**结论：经典 AI 词表（不是而是/仿佛/一丝/嘴角勾起/深吸一口气/眼中闪过等）全线 ≈0，防线有效；残留 AI 味在结构层**——① 视线词族（盯着/看向/目光）1.8-2.9/千字（凝视循环）；②「没有+动词」刹车句 0.92/千字 + 单句段 29-31%；③ 章末装置复用（陌生号码短信×4、文字收尾 8/36 章、心理宣言收尾时间铺子 4/10）；④「猛地」窄库；⑤「像是」比喻青冥问道 49.7/万字重灾区（deslop 已有密度规则覆盖） |
| 2026-08-29 | AI 味结构层对策 | ZCode | GUI b145e6b / TUI 3d3ba33 | **deslop 新增 5 条结构规则**：gaze-density（>1.5/千字 advisory、>5 blocking，阈值对标「健康线 ≈1/千字」）/brake-sentence/brake-standalone-para/one-line-para（>45%）/mengdi-density——真实语料回放：时间当铺 5/10 章命中、改命笔记 15/54 命中、干净文本零误报；**写作 prompt**：新增规则 21（刹车句 ≤3 且独段 ≤1/单句段 ≤3/不是而是全书禁用/段落短句连用禁）、视线词章级配额 ≤4 + 凝视三连限 1 + 猛地 ≤1、文字类章末装置每 10 章 ≤1；**审校 prompt**：F 维新增结构与句式新鲜度快数（三指纹任一至多 marginal、两类 fail）+ 硬纪律 7。双端同步，dual_sync_check 0 漂移 |
| 2026-08-29 | 对策真机验证 | ZCode | 《时间当铺》第 6 章重写 | 删最短章（旧版 1121 字）→ 新代码全链重跑：草稿一次达标 2027 字、扫描建议 38 条（新规则起效）、审校通过、定稿 2028 字完本；deslop 复扫新章：视线 2.5/千字（1 advisory）、明喻密度 14（advisory）——问题可见化达成，完全清洁需多章迭代（属生成侧与模型的持续博弈，防线已就位） |
| 2026-08-29 | 去 AI 味方法论收集（网络） | ZCode + 子代理 | 报告入库（知识库） | 抓取并整合 4 篇方法论全文：CSDN 网文 AI 味总结（网文专项：开篇环境描写超标+单句形容词>3=编辑判定线、华丽空洞、对话不出人设）、香宇 8 特征（节奏指纹/红旗词表/六步工作流）、36氪 2026 手册（文风投喂法/三明治结构/三类事实必查证）、知乎 8 技巧；核心洞察：**检测工具看统计指纹（句长方差/困惑度/突发性/情感曲线），不看词表**。来源：[CSDN](https://blog.csdn.net/qq_52080901/article/details/148605845)、[知乎](https://zhuanlan.zhihu.com/p/692546989)、[香宇](https://xiangyugongzuoliu.com/ai-style-writing-8-common-giveaways/)、[36氪](https://m.36kr.com/p/3824601267196037) |
| 2026-08-29 | GitHub 去 AI 味 skill 侦察 | ZCode + 子代理 | 报告入库（知识库） | 摸清 13 仓库，深挖 3 个：**blader/humanizer**（38.6k★，维基 35 条规律全文，含「误判防护」清单与小说豁免条款）、**zenstory-ai/oh-story-claudecode**（6.2k★，网文专用：10 模式+三遍法+毒级禁用词表，七猫 125 本爆款+番茄 305 章校准）、**lieflat-less-ai-tone**（283 万字实测：破折号 AI 2.38 vs 人类 0.80/千字、空转句 9.4 倍、拟人职业喻体 7.3 倍；**重要反转：句长均匀非 AI 指纹（0.87×）、比喻/问句人类用得更多**）。另有 shuorenhua（中文禁语全表）、speak-human-tw（38 痕迹）可后续引入 |
| 2026-08-29 | 《规则之外》真实窗口 10 章 | ZCode 驱动 GUI | 19,711 字完稿 | 全新项目（规则怪谈题材，灵感预埋 10 章结构）真实窗口自动档完稿；期间暴露并修复 save_state 的 os.replace Windows 文件锁（WinError 5，重试 3 次退避，GUI b2dd011 / TUI 8f800f0），10 章 history 无损 |
| 2026-08-29 | 《规则之外》深度 AI 味评估 | ZCode + 2 子代理 | 评估报告（结论如下） | **量化对比（三书实测）**：视线词族 时间当铺 2.64→规则之外 **0.96**/千字（-64%，新配额生效）、比喻 0.74→0.36、猛地 0.21→0.10；**指纹迁移实锤**：「没有X」刹车句 0.16→0.61、嘴角 0.05→0.61、精确时刻开场 0.66 独有——压旧口头禅冒新口头禅，博弈动态化。**6-10 章精读**：词法层 AI 味=低、结构层=中高、硬伤 2 处（高）：①**章节号泄漏进正文**（角色台词说出「第5章回溯」——管线把细纲章节号喂进上下文）；②反派数字链自爆（70 岁 vs 240 年寿命消耗）；结构层前三：设定朗读症（35-50% 对话在解释规则）、模板复用（同款开场 5 连/「数字变成N」×8/单词定性段家族）、后段衰减（第 9 章 1776 字谷底、独句段 45%）。注：1-5 章精读代理 70 分钟未归（超时），其量化覆盖已由全书实测补齐 |
| 2026-08-29 | 评估驱动修复（四项） | ZCode | GUI edd1c68 / TUI 0749726 | ①**章节号泄漏**：组装层 `_sanitize_chapter_refs` 清洗细纲/摘要的「第N章」标记（双端+离线验证）；②deslop 新增 stamp-para（单词定性独句段 ≥3 advisory）；③审校 D 维 V8 属性数字对账（年龄/寿命/次数链自洽）；④写作 prompt 禁设定朗读症（规则解释不得审讯式问答交付，每段对话 ≤1 个新设定点）+台词人设可区分（删名辨人）+外貌特征禁逐字复用（全书 ≤2 次） |
| 2026-08-29 | 去味综合方案（后续路线） | ZCode | 规划 | 短期已落地（上四项）；中期候选：①引入 oh-story 的 check-ai-patterns.js 改造为第六道 lint（毒级词表直译）；②审校槽新增「节奏指纹统计」（句长变异系数/独句段占比自动报告，当前仅 deslop 层）；③对话人设卡（角色语言画像注入 prompt，根治台词不可区分）；长期：①生成-评估闭环（每章生成后自动跑 deslop+审校双报告，超标自动重写——需防「二修伪自然」）；②按 lieflat 方法论用自产语料校准全部阈值（当前阈值来自公开基准+少量自产数据）；③文风样本投喂（36氪文风锚点法，让用户自己的文风成为防腐剂） |
