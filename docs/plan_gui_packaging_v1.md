...
| 2026-08-31 | 安装包补齐 + latest.json 预备 | Qoder | 97ba2c5 | ①Inno Setup 6.7.3 装入本机（GitHub 直连不通，经 api.github.com 下载并校验大小+Authenticode 后静默安装）；②**Inno 6.7+ 不再内置翻译**——ChineseSimplified.isl 随仓库分发（`tools/ChineseSimplified.isl`），installer.iss 改相对引用；③ISCC 通过：`dist/release/v0.14.0/QianBi-Novel-v0.14.0-setup.exe` 50.8MB（per-user 免管理员）；④安装冒烟自动化通过：静默安装→%LocalAppData%\Programs→启动存活（日志 v0.14.0）→静默卸载目录清空→`~/.qianbi_novel` 保留；⑤build_release.py 修正 SHA256SUMS 时序（移到安装包后、排除自身）；⑥`latest.json` 落仓库根（真实 setup.exe 哈希），本地 HTTP 验证 0.13.0 提示/0.14.0 静默。遗留：签名待证书；latest.json 待仓库公开推送后生效 |
