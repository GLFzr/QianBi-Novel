...
## C-4 校准目标清单（N-35 冲突对已定位，C-2 实测口径出来后定点改）

| 位置 | 现文 | 问题 |
|---|---|---|
| `README.md:56` | 对比表费用行「真实 e2e 写 5 章 ≈ ¥0.07」 | 与 CHANGELOG 差 18 倍；且口径（模型/槽位/章数）未标注 |
| `CHANGELOG.md:79` | v0.19 条目「成本 **¥0.25/章**（离峰）」 | 同上，两处必须统一到同一实测口径（附账本片段） |

> 其余 README 中的成本数字（:29-31 命中率/-29%、:273/:281 清算对照）均带版本/对照
> 标注，属历史口径陈述，不在本次校准范围。
