...
- 事实：#42 装配点在 Phase 1/3 的并集架构下**逐字未变**（静态段走卷级冻结头，
  不在轮次装配点计字符串内）；且 DESLOP_REWRITE_PROMPT 的「改写原则」段本就
  **不含**「仿佛」字样（v3 更正：此前的「模板删字」说法不成立）。4045→4002
  （−43）的真实机理是**检出降级**——CLICHE_SIMILE 从一级禁用词（逐词 blocking）
  降为密度型 advisory 后，扫描器不再产出「一级禁用词『仿佛』」类 finding
  （deslop.py 的该行输出消失），装配点 findings 文本随之变短 43 字。并集有
  8,108 字，若真并入装配点方向应为正向大增——「−43 归因于并入并集」在数量级
  和方向上都不成立。
